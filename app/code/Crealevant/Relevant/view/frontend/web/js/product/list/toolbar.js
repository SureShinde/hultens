...
define([
    ...
    "ko",
    "js/relevant-settings",
    "Magento_Catalog/js/product/list/toolbar",
    'mage/translate',
    'js/infinite',
    'js/bootstrap'
], function($, ui, messageComponent, ko, setting) {
    /**
    ...
    $.widget('mage.productListToolbarForm', $.mage.productListToolbarForm, {
        ...
        options:
            {
                ...
                layeredNavigationFilterBlock: '.block.filter:not("#mm-filters")',
                filterItemControl: '.block.filter:not("#mm-filters") .item a:not(".mm-next"), .block.filter:not("#mm-filters") li a:not(".mm-next"), .block.filter:not("#mm-filters") .filter-clear,.block.filter:not("#mm-filters") .swatch-option-link-layered, .pages-items a',
                url: ''
            },
        ...
        _updateContent: function (content) {
            $(this.options.productsToolbarControl).hide();
            $(window).resize(function () {
                if ($(window).width() < 992) {
                    $(".toggle-filter").hide();
                    $(".toggle-filter").last().show();
                } else {
                    $(".toggle-filter").hide();
                    $(".toggle-filter").last().hide();
                }
            });
            if ($(window).width() < 992) {
                $(".toggle-filter").hide();
        ...
        updateContent: function (content) {
            ...
            $('html, body').animate(
                {
                    scrollTop: $("#maincontent").offset().top - 200
                },
                100,
                'swing',
