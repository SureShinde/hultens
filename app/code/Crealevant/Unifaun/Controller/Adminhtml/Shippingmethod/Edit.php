...
class Edit extends \Mediastrategi\Unifaun\Controller\Adminhtml\Shippingmethod\Edit
{
    public function execute()
    {
        ...
        if (!empty($shippingMethod->getData())) {
            ...
            $extra = ($this->getRequest()->getParam('extra') && json_decode($this->getRequest()->getParam('extra'),
                true)
            ...
            if (!empty($title)
                ...
                if (isset($_FILES['image']['name']) && $_FILES['image']['name'] != '') {
                    ...
                    if ($uploader->checkAllowedExtension($ext)) {
                        ...
                        return $resultRedirect->setPath('/shippingmethod/edit/id/'. $id);
                    }
                } elseif (isset($image['delete']) && $image['delete'] == 1) {
                    $shippingMethod->setData('image', '');
                }

                try {
                    $shippingMethod->save();
                    $this->messageManager->addSuccess(__('You updated the shipping method.'));
                } catch (\Exception $e) {
            ...
            $resultPage->setActiveMenu('Mediastrategi_Unifaun::shippingmethod_edit');
            $resultPage->getConfig()->getTitle()->prepend(__('Edit Shipping Method'));
            ...
            $this->messageManager->addError(__('Failed to find shipping method'));
            /** @var \Magento\Backend\Model\View\Result\Redirect $resultRedirect */
            $resultRedirect = $this->resultRedirectFactory->create();
            return $resultRedirect->setPath(
                'msunifaun/shippingmethod/index'
