...
namespace Crealevant\MageWorxOptionAdvancedPricing\Plugin;
...
class ExtendPriceConfig extends \Magento\Catalog\Block\Product\View\Options
{
    ...
    public function getExtendedJsonConfig($defaultConfig, $option, $valueId)
    {
        ...
        if ($valueId !== null) {
            /* Begin get Final Price when apply catalog price rule */
            if ($option['type'] == 'drop_down') {
                foreach ($option->getValues() as $v) {
                    if ($v['option_type_id'] == $valueId) {
                        if ($sku = $v->getSku()) {
                            if ($finalPrice = $this->getOptionPrice($sku)) {
                                if ($finalPrice < $defaultConfigOptionId[$valueId]['prices']['basePrice']['amount']) {
                                    $defaultConfigOptionId[$valueId]['prices']['finalPrice']['amount'] = $finalPrice;
                                }
                            }
                        }
                    }
            ...
            /* End get Final Price when apply catalog price rule */
            $config['prices']['basePrice']['amount'] =
    ...
    private function getOptionPrice($sku)
    {
        try {
        ...
        return $this->pricingHelper->currency($_product->getPriceInfo()->getPrice('final_price')->getValue(), false, false);
    }
}
