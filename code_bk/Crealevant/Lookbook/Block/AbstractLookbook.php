            ...
            foreach($arrPin as $pin){
                ...
                if(trim($pin['custom_text'])!=''){
                    ...
                    if($product = $this->getProductInfo($pin['text'])){
                        ...
                        $html .= '<div><a data-cat-url="'.$this->getUrl().'" data-product-url="'.$product->getProductUrl().'" data-quickview-url="'. $this->getUrl('crealevant_quickview/catalog_product/view/', array('id' => $product->getId())).'" class="crealevant-quickview" href="javascript:void(0)">' . __('Detail') . '</a>';
