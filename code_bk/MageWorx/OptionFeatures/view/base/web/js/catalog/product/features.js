            ...
            $options.filter('select').each(function (index, element) {
                ...
                if ($element.hasClass('mageworx-swatch')) {
                    ...
                    $swatches.each(function (swatchKey, swatchValue) {
                        ...
                        var oldPrice = '';
                        var isSpecial = false;
                        var optionPriceValue = $(swatchValue).attr('data-option-value-price');
                        var optionFinalPrice = $(swatchValue).attr('data-option-final-price');
                        var optionsIsAppyRule = $(swatchValue).attr('data-option-is-apply-rule');
                        var optionIsSpecial = $(swatchValue).attr('data-option-is-special');
                        var valueId = $(swatchValue).attr('data-option-type-id');
                        var deliveryData = $(swatchValue).attr('data-option-delivery-time');
                        if (!_.isUndefined(value[valueId]) &&
                            ...
                            var title = '<div class="title">' + value[valueId]['title'] + '</div>' + '<div class="delivery-time">' + deliveryData + '</div>';
                            if(optionsIsAppyRule !==''){
                                var isSpecial = true;
                                var specialPrice = '<div class="special-price">' + optionFinalPrice + '</div>';
                                var oldPrice = '<div class="old-price">' + optionPriceValue + '</div>';
                            }

                            else if(optionIsSpecial !==''){
                                var isSpecial = true;
                                var specialPrice = '<div class="special-price">' + optionFinalPrice + '</div>';
                                var oldPrice = '<div class="old-price">' + optionPriceValue + '</div>';
                            }

                            var popupContentWithSpecial = '<div class="title">' + value[valueId]['title'] + '</div>' + oldPrice + specialPrice + '<div class="delivery-time">' + deliveryData + '</div>';
                            var popupContent = '<div class="title">' + value[valueId]['title'] + '</div>' + '<div class="price">' + optionPriceValue + '</div>' + '<div class="delivery-time">' + deliveryData + '</div>';
                            var description = '';
                            ...
                            var optionPopupContent =  tooltipImage + popupContent + stockMessage + description;
                            var optionPopupContentSpecial = tooltipImage + popupContentWithSpecial + stockMessage + description;

                            if(isSpecial){
                                $(swatchValue).parent().append('<div class="option-popup-information">'+optionPopupContentSpecial+'</div>');
                            }
                            else{
                                $(swatchValue).parent().append('<div class="option-popup-information">'+optionPopupContent+'</div>');
                            }
                        }
                    });
                } else {
