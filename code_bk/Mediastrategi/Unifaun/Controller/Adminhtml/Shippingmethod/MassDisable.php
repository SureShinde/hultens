...
class MassDisable extends \Magento\Framework\App\Action\Action
{
    ...
    public function execute()
    {
        ...
        foreach ($collection as $item) {
            ...
            if (!empty($shippingMethod->getData())) {
                $shippingMethod->setData('active', false);
                try {
                    $shippingMethod->save();
                    $updated++;
                } catch (\Exception $e) {
        ...
        if ($updated) {
            $this->messageManager->addSuccess(__('Disabled the shipping methods.'));
        } else {
            $this->messageManager->addError(__('Failed to disable the shipping methods'));
        }
    ...
    {
        return $this->_authorization->isAllowed('Mediastrategi_Unifaun::shippingmethod_massDisable');
    }
}
